...
function getNextFriday(date) {
  ...
  const dayToAdd = (5 - currentDay + 7) % 7 || 7;
  const res = new Date(date);
  res.setDate(date.getDate() + dayToAdd);
  return res;
}
...
function formatDate(date) {
  ...
  return `${newDate.getUTCMonth() + 1}/${newDate.getUTCDate()}/${newDate.getUTCFullYear()}, ${hours}:${minutes}:${seconds} ${daytime}`;
}
...
function getCountWeekendsInMonth(month, year) {
  const TARGET_DAYS = [6, 0];
  let count = 0;
  const daysInMonth = getCountDaysInMonth(month, year);
  for (let i = 1; i <= daysInMonth; i += 1) {
    if (TARGET_DAYS.includes(new Date(year, month - 1, i).getDay())) {
      count += 1;
    }
  }
  return count;
...
function getWeekNumberByDate(date) {
  ...
  const offset =
    firstWeekDay >= 5 || firstWeekDay === 0 ? 7 + 1 - (firstWeekDay || 7) : 0;
  firstDay.setDate(firstDay.getDate() + offset);
  return Math.floor((date - firstDay) / (1000 * 60 * 60 * 24) / 7) + 1;
}
